...
def primes_list_sieve(n):
    ...
    for i in range(3, int(math.sqrt(n + 1)) + 1, 2):
        if s[i]:
            s[i * i::2 * i] = [False] * ((n - i * i) // (2 * i) + 1)
    return [2] + [i for i in range(3, n + 1, 2) if s[i]]
...
if __name__ == '__main__':
    try:
        ...
        print('Error while calling "int()": the entered string is not an integer.', file=sys.stderr)
        print(e, file=sys.stderr)
        exit(1)
    except Exception as e:
        print('Error while calling "int()"', file=sys.stderr)
        print(e, file=sys.stderr)
        exit(1)
